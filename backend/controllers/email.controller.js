...
const emailController = {
    sendVerificationEmail: async (recipientEmail, username, verificationToken) => {
        ...
        const mailOptions = {
            ...
            html: htmlContent.replace('{{emailVerificationLink}}', verificationLink).replace('{{username}}', username),
        };
    ...
    forgotPassword: async (req, res) => {
        try{
            ...
            return res.status(200).json({message: 'Email is successfully send.'});
        }
        catch(error){
            return res.status(500).json({
                error: 'Internal server error'
